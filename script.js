...
// look for passengers function
// create manifest of travlers(waiting)
// fix pickup 

// let waiting = {
//     2: [new Person(2,5), new Person(2,6)],
//     3: [new Person(3,1)],
//     4: [new Person(4,8)],
//     5: [new Person(5,1)],
//     6: [new Person(6,0)]
...
function dropoff() {
    ...
    if (Object.keys(waiting).length === 0 && Object.keys(elevator.passengers).length === 0){
        ...
        elevator.state = 'checking';
        if ( elevator.direction == 'up'){
            for(let i = floors.length; i >elevator.currentFloor; i--){
            ...
            for(let i = 0; i < elevator.currentFloor; i++){
                if(waiting[i]== undefined){
                    continue;
                }
                else
...
// find intial passenger on start click to determine direction 
// solve intial and then add people
